...
contract PuppetV2Pool {
    ...
    function borrow(uint256 borrowAmount) external {
        require(_token.balanceOf(address(this)) >= borrowAmount, "Not enough token balance");   // Pool이 현재 소유한 DTV 토큰의 수량이하로만 대출 가능
    ...
    /**
     * @notice Uniswap V2 의 Price Oracle에 기반하여, V2 Exchange의 변동될 토큰 수에 따른 예치할 WETH 수량을 계산
     * @param tokenAmount 대출받을 DTV 토큰의 수량
     * @return 예치해야할 WETH 토큰의 수량
     */
    function calculateDepositOfWETHRequired(uint256 tokenAmount) public view returns (uint256) {
        return _getOracleQuote(tokenAmount).mul(3) / (10 ** 18);        // Price Oracle로부터 획득한 값의 x3 만큼의 예치할 ETH 수량 필요
                                                                        // Price Oracle은 CPMM에 의해 계산된 값! 
    }
    ...
    function _getOracleQuote(uint256 amount) private view returns (uint256) {
        ...
        return UniswapV2Library.quote(amount.mul(10 ** 18), reservesToken, reservesWETH);   // 변동될 토큰 수에 따른 WETH(예치할) 수량 계산
    }
}
